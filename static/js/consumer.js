...
function renderCards(photos,reset){
  ...
  photos.forEach((p,i)=>{
    ...
    const rating=p.avg_rating?`<span style="color:#4F46E5">★ ${parseFloat(p.avg_rating).toFixed(1)}</span>`:'';
    card.innerHTML=`
...
function starsHTML(r,size=14){return[1,2,3,4,5].map(i=>`<span style="font-size:${size}px;color:${i<=Math.round(r)?'#F59E0B':'var(--text-3)'}">★</span>`).join('');}
